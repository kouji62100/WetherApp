...
public class WeatherFragment3 extends Fragment {
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        WeatherApp.get()
                .getRequestQueue()
                .add(WeatherDataRequest.get(helper.getAreaData().get(2).getLat(), helper.getAreaData().get(2).getLon(),new WeatherDataRequest.WeatherDataRequestResponseListener(){
                    @Override
                    public void onResponse(WeatherDataEntity response){
                        //Viewの表示処理
                        TextView area_name1 = (TextView)getActivity().findViewById(R.id.area_name1);
                        area_name1.setText(helper.getAreaData().get(2).getAreaName());
                    }
